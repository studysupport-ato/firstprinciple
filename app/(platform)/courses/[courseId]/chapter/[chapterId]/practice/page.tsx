...
import { createPracticeSession, evaluateQuestionAnswer, getPracticeQuestions, getPracticeSummary, type PracticeSession, type PracticeValue } from "@/lib/practice/session";
...
export default function ChapterPracticePage() {
  ...
  const difficulty: Difficulty | undefined = difficultyValue === "easy" || difficultyValue === "medium" || difficultyValue === "hard" ? difficultyValue : undefined;
  const requestedLimit = Number(searchParams.get("limit"));
  const limit = Number.isFinite(requestedLimit) && requestedLimit > 0 ? Math.min(20, Math.round(requestedLimit)) : 5;
  ...
  useEffect(() => {
    const selected = getPracticeQuestions({ courseId, chapterId, topic, subtopic, difficulty, limit, includeDraft: preview });
  ...
  }, [chapterId, courseId, difficulty, limit, preview, subtopic, topic]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#F7F7F8] p-6">
        <div className="max-w-lg rounded-[30px] border border-[#E5E5E5] bg-white p-10 text-center shadow-sm">
          ...
          <p className="mt-4 font-sans text-sm leading-relaxed text-[#666666]">This chapter does not yet have a local practice set in the structured question bank.</p>
  ...
  const handleReset = () => {
    const freshQuestions = getPracticeQuestions({ courseId, chapterId, topic, subtopic, difficulty, limit, includeDraft: preview });
