...
function QuestionRenderer({
  ...
  return (
    <div className="space-y-8">
      ...
      {submitted && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="overflow-hidden rounded-2xl border border-[#E5E5E5] bg-transparent p-5">
      ...
      <div className="min-h-screen bg-transparent p-6 md:p-10">
        <div className="mx-auto max-w-5xl rounded-[30px] border border-[#E5E5E5] bg-white p-6 shadow-sm md:p-10">
          ...
          <div className="grid gap-6 md:grid-cols-[1.2fr_2fr]">
            <div className="rounded-2xl border border-[#E5E5E5] bg-transparent p-6">
    ...
    <div className="flex min-h-screen flex-col bg-transparent">
      ...
      <main className="flex flex-1 items-center justify-center p-6">
        <div className="w-full max-w-5xl rounded-[30px] border border-[#E5E5E5] bg-white p-6 shadow-sm md:p-10">
          <QuestionRenderer question={currentQuestion} selectedValue={currentAnswerValue} onSelect={handleAnswerSelect} submitted={submitted} />
          ...
          <div className="mt-8 grid gap-4 border-t border-[#E5E5E5] pt-6 md:grid-cols-[1fr_auto]">
            ...
            <div className="flex items-center justify-end gap-3">
              <button type="button" onClick={handlePrevious} disabled={session.currentIndex === 0} className="rounded-full border border-[#E5E5E5] px-5 py-3 font-sans text-sm font-semibold text-[#666666] transition hover:border-[#111111] hover:text-[#111111] disabled:cursor-not-allowed disabled:opacity-40">
                Previous
              </button>

              {!showConfirm ? (
                <button type="button" onClick={() => (session.currentIndex === questions.length - 1 ? setShowConfirm(true) : handleNext())} className="inline-flex items-center gap-2 rounded-full bg-[#111111] px-7 py-3 font-sans text-sm font-semibold text-white transition hover:bg-[#2563EB]">
                  {session.currentIndex === questions.length - 1 ? "Submit" : "Next"} <ArrowRight size={16} />
                </button>
              ) : (
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => setShowConfirm(false)} className="rounded-full border border-[#E5E5E5] px-5 py-3 font-sans text-sm font-semibold text-[#666666]">
                    Cancel
                  </button>
                  <button type="button" onClick={handleSubmit} className="rounded-full bg-[#111111] px-6 py-3 font-sans text-sm font-semibold text-white transition hover:bg-[#2563EB]">
                    Confirm submit
                  </button>
                </div>
              )}
