...
import { recordAssessmentStart, recordAssessmentSubmit } from "@/lib/progress";
...
export default function AssessmentPage() {
  ...
  const courseId = (params.courseId as string) ?? "math-151";
  const chapterId = (params.chapterId as string) ?? "complex-numbers";
  const assessmentId = (params.assessmentId as string) ?? "math151-complex-numbers-checkpoint";
  ...
  useEffect(() => {
    const assessment = getStudentAssessment(assessmentId, { preview });
    if (!assessment) return;

    const selected = selectAssessmentQuestions(assessment, { includeDraft: preview });
    const nextSession = createAssessmentSession(assessment.id, assessment.courseId, selected.map((question) => question.id), chapterId);
    setQuestions(selected);
    setSession(nextSession);
    setSelectedValue(null);
    setShowConfirm(false);
    setResult(null);
    recordAssessmentStart(assessment.courseId, assessment.id, chapterId, nextSession.startedAt);
  }, [assessmentId, chapterId, preview]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#F7F7F8] p-6">
        <div className="max-w-lg rounded-[30px] border border-[#E5E5E5] bg-white p-10 text-center shadow-sm">
          ...
          <p className="mt-4 font-sans text-sm leading-relaxed text-[#666666]">The selected blueprint did not resolve to a valid local assessment.</p>
